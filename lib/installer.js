...
function installCunzhi() {
  ...
  if (platform === 'darwin') {
    ...
    const binDir = path.join(HOME, '.local', 'bin');
    fs.mkdirSync(binDir, { recursive: true });
    const zipPath = path.join(os.tmpdir(), 'cunzhi.zip');
    execSync(`curl -fsSL -o "${zipPath}" "https://github.com/imhuso/cunzhi/releases/latest/download/cunzhi-cli-windows-x86_64.zip"`, { stdio: 'pipe' });
    execSync(`powershell -Command "Expand-Archive -Path '${zipPath}' -DestinationPath '${binDir}' -Force"`, { stdio: 'pipe' });
    fs.unlinkSync(zipPath);
    console.log(chalk.yellow(`\n  ⚠️  请将 ${binDir} 添加到系统 PATH`));
  } else {
